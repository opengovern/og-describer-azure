...
type monitoringMetric struct {
	...
	MetaData *armmonitor.MetricAggregationType
...
func kaytuCommonMonitoringMetricColumns() []*plugin.Column {
	return []*plugin.Column{
		...
		},
		{
			...
			Description: "The units in which the metric value is reported.",
			Type:        proto.ColumnType_STRING,
			Transform:   transform.FromField("Description.Unit"),
		},
		{
			Name:        "cloud_environment",
			Description: ColumnDescriptionCloudEnvironment,
		...
		},
		{
			...
			Description: ColumnDescriptionResourceGroup,
			Type:        proto.ColumnType_STRING,
			Transform:   transform.FromField("Description.DimensionValue").Transform(extractResourceGroupFromID),
		},
		{
			Name:        "subscription_id",
			Description: ColumnDescriptionSubscription,
...
func listAzureMonitorMetricStatistics(ctx context.Context, d *plugin.QueryData, granularity string, metricNameSpace string, metricNames string, dimensionValue string, cred *azidentity.ClientSecretCredential) (interface{}, error) {
	...
	monitoringClient, err := armmonitor.NewMetricsClient(subscriptionID, cred, nil)
	if err != nil {
		return nil, err
	}
	...
	result, err := monitoringClient.List(ctx, dimensionValue, &options)
	if err != nil {
		return nil, err
	}
